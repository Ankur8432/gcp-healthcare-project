...
CREATE TABLE IF NOT EXISTS `avd-group-gcp.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `avd-group-gcp.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `avd-group-gcp.silver_dataset.claims` c
